...
from mypyftpdlib.ioloop import Acceptor
from mypyftpdlib.ioloop import Connector
...
class ActiveDTP(Connector):
    ...
    def handle_connect(self):
        ...
        self.cmd_channel.log_cmd(self._cmd, self._normalized_addr, 200, "已建立主动数据连接")
        if not self.cmd_channel.connected:
            return self.close()
        # delegate such connection to DTP handler
    ...
    def handle_timeout(self):
        if self.cmd_channel.connected:
            ...
            self.cmd_channel.log_cmd(
                self._cmd, self._normalized_addr, 421, "主动数据连接超时，关闭连接"
            )
        self.close()
    ...
    def handle_close(self):
        ...
        if not self._closed:
            ...
            if self.cmd_channel.connected:
                ...
                self.cmd_channel.log_cmd(
                    self._cmd, self._normalized_addr, 425, "无法连接到指定地址"
                )
